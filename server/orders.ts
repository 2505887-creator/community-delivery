...
router.post('/', requireAuth(['tenant', 'provider', 'driver', 'merchant', 'admin']), async (req: AuthedRequest, res: Response) => {
  try {
    ...
    if (!body.title || !body.tenantAddress) {
    ...
    const created = await prisma.order.create({
      data: {
        ...
        tenantPhone: String(body.tenantPhone || 'Not provided'),
    ...
    if (created.tenantUserId) await createNotification({userId: created.tenantUserId,type:'order.created',title:'Order created',message:`${created.title} has been created.`,data:{orderId:created.id}});
    if (created.provider?.userId) await createNotification({userId:created.provider.userId,type:'order.assigned',title:'New service request',message:`${created.title} is assigned to you.`,data:{orderId:created.id}});
...
router.patch('/:orderId/assign', requireAuth(['provider', 'admin']), async (req: AuthedRequest, res: Response) => {
  try {
    ...
    const assignedPro = await prisma.verifiedPro.findUnique({where:{id:proId},select:{userId:true,name:true}});
    if (order.tenantUserId) await createNotification({userId:order.tenantUserId,type:'order.assigned',title:'Provider assigned',message:`${assignedPro?.name||'A provider'} accepted your request.`,data:{orderId}});
    if (assignedPro?.userId) await createNotification({userId:assignedPro.userId,type:'order.assigned',title:'Job assigned',message:`${order.title} is now assigned to you.`,data:{orderId}});
...
router.patch('/:orderId/status', requireAuth(['provider', 'driver', 'admin']), async (req: AuthedRequest, res: Response) => {
  try {
    ...
    if (order.tenantUserId) await createNotification({userId:order.tenantUserId,type:'order.status',title:'Order status updated',message:`${order.title}: ${String(status).replaceAll('_',' ')}.`,data:{orderId,status}});
    return res.json({ success: true, data: serializeOrder(updated) });
  } catch (error) {
